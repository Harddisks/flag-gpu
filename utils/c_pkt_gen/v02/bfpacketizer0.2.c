...
unsigned long long packet_count = 0x0; //Global packet_count for thread synch 
...
void header_gen(unsigned char *packet_header, unsigned long long packet_count, 
                unsigned char m, unsigned char k, unsigned char cal)
{
	int p;
        unsigned long long header;
	cal = (cal & 0xF);
	//Fill header with packet_count, cal, m and k
	header = packet_count;
	header = ((header << 4) | cal);
	header = ((header << 8) | m);
	header = ((header << 8) | k);
...
void* send_packet(void *td)
{
	struct send_packet_data* ThreadData = (struct send_packet_data*) td;
   	...
   	int k, rv;
    	unsigned long long lpacket_count=0;
    	double byte_count=0;
        ...
        while (run && (byte_count<total_data || total_data<=0)) {
		   ...
		   header_gen(packet, lpacket_count, ThreadData->m, 
		   ...
		   if(ThreadData->UseGlobalPacketCount){
		      packet_count++;
		      lpacket_count = packet_count;
		   }
		   else{
		      lpacket_count++;
		   }
...
void main(int argc, char *argv[])
{
   ...
   /* Send packets */
	...
	struct send_packet_data data_thread[Mmax];
   	...
   	int RoachUsingGlobalPacketCount = -1;

   	for (mcnt=0; mcnt<Mmax; mcnt++) {
       		/* Create socket */
       		sock[mcnt] = socket(PF_INET, SOCK_DGRAM, 0);
       		if (sock[mcnt]==-1) {
	   		perror("socket");
	   		exit(1);
       		}
		//Data for each thread is set here
		data_thread[mcnt].m = mcnt; 
		data_thread[mcnt].cal = cal; 
		data_thread[mcnt].Kmax = Kmax; 
		data_thread[mcnt].SineCh = SineCh;
		data_thread[mcnt].SinePh = SinePh;
		data_thread[mcnt].SinePer = SinePer; 
   	        if(mcnt == RoachUsingGlobalPacketCount){
		    data_thread[mcnt].UseGlobalPacketCount = 1;
                }
                else{
		    data_thread[mcnt].UseGlobalPacketCount = 0;
                }
		//Create Thread
		pthread_create(&thread_send_packet[mcnt], NULL, (void*)send_packet, &data_thread[mcnt]);
